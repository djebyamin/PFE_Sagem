...
import { CalendarDays, Clock, CheckCircle, Bell, BarChart, Activity, Award, ArrowUp, Sparkles, Search, Users, FileText, Settings, Calendar, MessageSquare, BarChart3, PieChart as PieChartIcon, TrendingUp, AlertCircle, Briefcase, Target, UserPlus, Filter, Download, Share2, ChevronDown, MapPin, LogOut } from "lucide-react"
...
import { getOrCreateConversation, getMessages, sendMessage, getUtilisateurs } from './actions'
...
export default function Dashboard({ user, utilisateurs: initialUtilisateurs }: ManagerComponentProps) {
  ...
  const [lastMessageTime, setLastMessageTime] = useState<Record<number, Date>>({})
  const audioRef = useRef<HTMLAudioElement | null>(null)

  // Charger les utilisateurs et les messages
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Si un utilisateur est sélectionné, charger ses messages
        if (selectedUser) {
          const conversation = await getOrCreateConversation(Number(user.id), selectedUser.id);
  ...
  // Rafraîchir les messages toutes les 10 secondes si une conversation est active
  useEffect(() => {
    ...
    if (chatConversationId) {
      interval = setInterval(async () => {
        try {
          ...
          console.error('Erreur lors du rafraîchissement des messages:', e);
        }
      }, 10000);
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !chatConversationId) return;
  ...
  // Ajout d'un state pour les messages non lus reçus
  const unreadReceivedMessages = messages.filter(
    msg => msg.unread && msg.sender !== user.nom + ' ' + user.prenom // non lu et pas envoyé par moi
  );
  ...
  // Ajouter un gestionnaire de clic en dehors de la fenêtre de messages
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (messagesContainer && 
          !messagesContainer.contains(event.target as Node) && 
          messagesButton && 
          !messagesButton.contains(event.target as Node)) {
  ...
  useEffect(() => {
    if (chatMessages.length > 0) {
      ...
      updateMessageStatus(lastMessage.id, 'delivered');
    }
  }, [chatMessages]);
          ...
          <div className="p-4">
            <div className="flex items-center gap-3 mb-6">
              <Avatar className="h-10 w-10">
                ...
                <AvatarFallback className="bg-blue-600">{user.nom.charAt(0)}{user.prenom.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                ...
                <div className="relative">
                  ...
                  {isMessagesOpen && (
                    <div 
                      ...
                      className="absolute right-0 mt-2 w-[800px] h-[600px] bg-white rounded-lg shadow-lg border border-gray-200 z-50 flex"
                    >
                      ...
                      <div className="flex-1 flex flex-col bg-white/70">
                        ...
                        <div className="p-4 border-t flex gap-2 bg-white rounded-b-2xl">
                          ...
                          <Button
                            ...
                            disabled={chatLoading || !newMessage.trim() || !selectedUser}
                          >
                            Envoyer
                          </Button>
                        </div>
